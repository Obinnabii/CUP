...
public class UnifiedExample {
     ...
     * @param nextSym
     */
    public UnifiedExample(lalr_state conflict, lalr_item itm1, lalr_item itm2, terminal nextSym) {
        ...
        // parser states.
        shortestConflictPath = findShortestPathFromStart(itm1, optimizeShortestPath);
        ...
        for (StateItem si : shortestConflictPath) {
            scpSet.add(si.state);
            reduceProdReached = reduceProdReached || si.item.the_production() == reduceProd;
     ...
     */
    protected List<StateItem> findShortestPathFromStart(lalr_item tgt, boolean optimized) {
        StateItem.init();
        ...
        Set<StateItem> eligible = optimized ? eligibleStateItemsToConflict(target) : null;
        ...
        Set<StateItemWithLookahead> visited = new HashSet<>();
        {
            ...
            init.add(
                    new StateItemWithLookahead(
                            source, StateItem.symbolSet(source.item.lookahead())));
            queue.add(init);
        }
        ...
        while (!queue.isEmpty()) {
            ...
            if (target.equals(last.si) && last.lookahead.contains(nextSym)) {
                ...
                List<StateItem> shortestConflictPath = new ArrayList<>(path.size());
                for (StateItemWithLookahead sil : path) shortestConflictPath.add(sil.si);
                return shortestConflictPath;
            }
            // Transition
            if (StateItem.trans.containsKey(last.si)) {
                for (Map.Entry<symbol, StateItem> trans : StateItem.trans.get(last.si).entrySet()) {
                    ...
                    List<StateItemWithLookahead> nextPath = new LinkedList<>(path);
                    nextPath.add(next);
                    queue.add(nextPath);
                }
            }
            ...
            if (StateItem.prods.containsKey(last.si)) {
                ...
                for (lalr_item itm : StateItem.prods.get(last.si)) {
                    ...
                    if (optimized && !eligible.contains(nextSI)) continue;
                    StateItemWithLookahead next = new StateItemWithLookahead(nextSI, lookahead);
                    List<StateItemWithLookahead> nextPath = new LinkedList<>(path);
                    nextPath.add(next);
                    queue.add(nextPath);
                }
    ...
    protected Set<StateItem> eligibleStateItemsToConflict(StateItem target) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (StateItem.revTrans.containsKey(si))
                for (Set<StateItem> prev : StateItem.revTrans.get(si).values()) queue.addAll(prev);
            if (si.item.dot_pos() == 0) {
                ...
                if (StateItem.revProds.containsKey(si.state)) {
                    Map<non_terminal, Set<lalr_item>> revProd = StateItem.revProds.get(si.state);
    ...
    protected Counterexample findExample() {
        SearchState initial =
                new SearchState(StateItem.lookup(conflict, itm1), StateItem.lookup(conflict, itm2));
        // Timer
        long start;
        ...
        while (!pq.isEmpty()) {
            ...
            for (SearchState ss : fcss.sss) {
                ...
                if (ss.reduceDepth < 0 && ss.shiftDepth < 0) {
                    ...
                    // Stage 3
                    stage = 3;
                    if (si1src.item.the_production().lhs().the_symbol()
                                    == si2src.item.the_production().lhs().the_symbol()
                        ...
                        if (ss.derivs1.size() == 1
                            ...
                            // looking for.
                            if (Main.report_cex_stats) System.err.println(ss.complexity);
                            return new Counterexample(ss.derivs1.get(0), ss.derivs2.get(0), true);
                        }
                ...
                if (timeLimitEnforced) {
                    if (!assurancePrinted
                            ...
                            && stage3result != null) {
                        System.err.println(
                                "Productions leading up to the conflict state found.  Still finding"
                                        + " a possible unifying counterexample...");
                        assurancePrinted = true;
                    }
                    if (System.nanoTime() - start > TIME_LIMIT) {
                        System.err.println("time limit exceeded: " + (System.nanoTime() - start));
                        if (Main.report_cex_stats && Main.report_cex_stats_to_out)
                            System.out.println("time limit exceeded");
                        if (stage3result != null) {
                            if (Main.report_cex_stats) System.err.println(stage3result.complexity);
                ...
                if (!si1reduce && !si2reduce) {
                    ...
                    if (si1sym == si2sym) {
                        ...
                        // symbol into account.
                        StateItem si1last = StateItem.trans.get(si1).get(si1sym);
                        StateItem si2last = StateItem.trans.get(si2).get(si2sym);
                        ...
                        nullableClosure(
                                si1.item.the_production(),
                                si1.item.dot_pos() + 1,
                                si1last,
                                states1,
                                derivs1);
                        nullableClosure(
                                si2.item.the_production(),
                                si2.item.dot_pos() + 1,
                                si2last,
                                states2,
                                derivs2);
                    ...
                    if (si1sym instanceof non_terminal && StateItem.prods.containsKey(si1))
                        for (lalr_item itm1 : StateItem.prods.get(si1)) {
                            ...
                            if (!StateItem.productionAllowed(prod, nextProd)) continue;
                            // Check for subsequent nullable nonterminals
                            ...
                            for (int i = 0, size1 = derivs1.size(); i <= size1; i++) {
                    ...
                    if (si2sym instanceof non_terminal && StateItem.prods.containsKey(si2))
                        for (lalr_item itm2 : StateItem.prods.get(si2)) {
                            ...
                            for (int i = 0, size2 = derivs2.size(); i <= size2; i++) {
                                List<Derivation> subderivs2 =
                    ...
                    if (ready1) {
                        ...
                        for (SearchState candidate : reduced2) add(pq, fcssMap, visited, candidate);
                    }
                    // Otherwise, prepend both paths and continue.
                    // This is preparing both paths for a reduction.
                    else {
                        symbol sym;
                        if (si1reduce && !ready1) sym = rhs(prod1, len1 - size1);
                        else sym = rhs(prod2, len2 - size2);
                        for (SearchState prepended :
                                ss.prepend(
                                        sym, null, null, ss.reduceDepth >= 0 ? rppSet : scpSet)) {
     ...
     */
    protected void nullableClosure(
            production prod,
            int pos,
            StateItem silast,
        ...
        for (int curPos = pos, len = prod.rhs_length(); curPos < len; curPos++) {
            ...
            derivs.add(new Derivation(nt, Collections.<Derivation>emptyList()));
            states.add(silast);
        }
    }
     ...
     */
    protected void add(
            PriorityQueue<FixedComplexitySearchState> pq,
     ...
     */
    protected void visited(Map<List<StateItem>, Set<List<StateItem>>> visited, SearchState ss) {
        Set<List<StateItem>> visited1 = visited.get(ss.states1);
        if (visited1 == null) {
    ...
    public Counterexample exampleFromShortestPath(boolean timeout) {
        ...
        for (; refsi != null; ) {
            ...
            if (prevrefsi != null) {
                for (int curPos = refsi.item.dot_pos(), prevPos = prevrefsi.item.dot_pos();
                        prevrefsi != null && prevPos + 1 != curPos; ) {
            ...
            if (si == refsi || si.item == lalr_state.startItem()) {
                ...
                while (itr.hasPrevious()) result.add(0, itr.previous());
                Derivation deriv1 = completeDivergingExample(shortestConflictPath);
            ...
            if (pos == 0) {
                ...
                while (!queue.isEmpty()) {
                    ...
                    if (srcpos > 0) {
                        ...
                        for (StateItem prevsi : StateItem.revTrans.get(sisrc).get(sym)) {
                        ...
                        production prod = sisrc.item.the_production();
                        symbol lhs = prod.lhs().the_symbol();
                        for (lalr_item prev : StateItem.revProds.get(sisrc.state).get(lhs)) {
                            StateItem prevsi = StateItem.lookup(sisrc.state, prev);
     ...
     * @return
     */
    protected Counterexample completeDivergingExamples(SearchState ss, boolean timeout) {
    ...
    protected Derivation completeDivergingExample(List<StateItem> states) {
        return completeDivergingExample(states, Collections.<Derivation>emptyList());
    }
     ...
     * @return A top-level derivation after completing all the productions.
     */
    protected Derivation completeDivergingExample(List<StateItem> states, List<Derivation> derivs) {
        ...
        for (ListIterator<StateItem> sItr = states.listIterator(states.size());
                sItr.hasPrevious(); ) {
            ...
            for (int i = pos + 1; i < len; i++) {
                ...
                if (lookaheadRequired) {
                    if (sym != nextSym) {
                        ...
                        non_terminal nt = (non_terminal) sym;
                        if (!nt.nullable() || nt.first_set().contains(nextSym)) {
                            List<Derivation> nextDerivs =
                                    expandFirst(StateItem.trans.get(si).get(rhs(prod, pos)));
                            ...
                            result.add(new Derivation(sym, Collections.<Derivation>emptyList()));
                        }
                    } else {
            ...
            for (int i = pos - 1; i >= 0; i--) {
                ...
                result.add(0, dItr.hasPrevious() ? dItr.previous() : new Derivation(rhs(prod, i)));
            }
    ...
    protected List<Derivation> expandFirst(StateItem start) {
            ...
            if (sym == nextSym) {
                ...
                for (ListIterator<StateItem> sItr = states.listIterator(states.size());
                    ...
                    if (pos == 0) {
                        ...
                        Derivation deriv =
                                new Derivation(
                                        rhs(prod, pos - 1), Collections.<Derivation>emptyList());
                        result.add(0, deriv);
                    }
                }
                result.remove(0);
        ...
        throw new Error(
                "Should not reach here (expected symbol: "
                        + nextSym
                        + "; item to be expanded: "
                        + start
                        + ").");
    ...
    protected class SearchState implements Comparable<SearchState> {
        ...
        private SearchState(
                List<Derivation> derivs1,
                List<Derivation> derivs2,
                List<StateItem> states1,
                List<StateItem> states2,
                int complexity,
                int reduceDepth,
                int shiftDepth) {
        ...
        protected SearchState copy() {
            return new SearchState(
                    derivs1, derivs2, states1, states2, complexity, reduceDepth, shiftDepth);
        }

        protected List<SearchState> prepend(symbol sym, symbol nextSym1, symbol nextSym2) {
         ...
         */
        protected List<SearchState> prepend(
                symbol sym, symbol nextSym1, symbol nextSym2, Set<lalr_state> guide) {
            ...
            Set<StateItem> prev1 =
                    extendedSearch
                            ? new HashSet<>(si1src.reverseTransition(sym, si1lookahead, guide))
                            : null;
            Set<StateItem> prev2 =
                    extendedSearch
                            ? new HashSet<>(si2src.reverseTransition(sym, si2lookahead, guide))
                            : null;
            List<StateItem> prev1ext =
                    si1src.reverseTransition(sym, si1lookahead, extendedSearch ? null : guide);
            List<StateItem> prev2ext =
                    si2src.reverseTransition(sym, si2lookahead, extendedSearch ? null : guide);
            for (StateItem psis1 : prev1ext) {
                ...
                for (StateItem psis2 : prev2ext) {
                    boolean guided2 = extendedSearch ? prev2.contains(psis2) : true;
                    ...
                    if (psis1 != null
                            && copy.states1.get(0).item.dot_pos() + 1
                                    == copy.states1.get(1).item.dot_pos()) {
                        if (psis2 != null
                                && copy.states2.get(0).item.dot_pos() + 1
                                        == copy.states2.get(1).item.dot_pos()) {
                    ...
                    int prependSize = (psis1 == null ? 0 : 1) + (psis2 == null ? 0 : 1);
                    int productionSteps =
                            (psis1 == null
                                            ? 0
                                            : productionSteps(
                                                    Collections.singletonList(psis1), si1src))
                                    + (psis2 == null
                                            ? 0
                                            : productionSteps(
                                                    Collections.singletonList(psis2), si2src));
                    copy.complexity +=
        ...
        protected List<SearchState> reduce1(symbol nextSym) {
            ...
            if (!item.dot_at_end()) throw new Error("Cannot reduce item without dot at end.");
            List<SearchState> result = new LinkedList<>();
            Set<symbol> symbolSet =
                    nextSym == null ? StateItem.symbolSet(item.lookahead()) : symbolSet(nextSym);
            if (!StateItem.intersect(item.lookahead(), symbolSet)) return result;
            production prod = item.the_production();
            symbol lhs = prod.lhs().the_symbol();
            int len = prod.rhs_length();
            int dSize = derivs.size();
            Derivation deriv =
                    new Derivation(lhs, new LinkedList<>(derivs.subList(dSize - len, dSize)));
            if (reduceDepth == 0) {
            ...
            if (sSize == len + 1) {
                ...
                List<List<StateItem>> prev = states.get(0).reverseProduction(symbolSet);
                for (List<StateItem> psis : prev) {
                    SearchState copy = copy();
                    copy.derivs1 = derivs;
                    copy.states1 = new LinkedList<>(states1.subList(0, sSize - len - 1));
                    copy.states1.addAll(0, psis);
                    copy.states1.add(
                            StateItem.trans
                                    .get(copy.states1.get(copy.states1.size() - 1))
                                    .get(lhs));
                    int statesSize = copy.states1.size();
                    int productionSteps = productionSteps(copy.states1, states.get(0));
                    copy.complexity +=
                ...
                copy.states1 = new LinkedList<>(states1.subList(0, sSize - len - 1));
                copy.states1.add(
                        StateItem.trans.get(copy.states1.get(copy.states1.size() - 1)).get(lhs));
            ...
            for (SearchState ss : result) {
                ...
                List<StateItem> states1 = new LinkedList<>();
                nullableClosure(
                        next.item.the_production(), next.item.dot_pos(), next, states1, derivs1);
        ...
        protected List<SearchState> reduce2(symbol nextSym) {
            ...
            if (sSize == len + 1) {
                ...
                for (List<StateItem> psis : prev) {
                    SearchState copy = copy();
                    copy.derivs2 = derivs;
                    copy.states2 = new LinkedList<>(states.subList(0, sSize - len - 1));
                    copy.states2.addAll(0, psis);
                    copy.states2.add(
                            StateItem.trans
                                    .get(copy.states2.get(copy.states2.size() - 1))
                                    .get(lhs));
                    int statesSize = copy.states2.size();
                    int productionSteps = productionSteps(copy.states2, states.get(0));
                    copy.complexity +=
                ...
                copy.states2 = new LinkedList<>(states.subList(0, sSize - len - 1));
                copy.states2.add(
                        StateItem.trans.get(copy.states2.get(copy.states2.size() - 1)).get(lhs));
                copy.complexity += REDUCE_COST;
                if (copy.shiftDepth >= 0) copy.shiftDepth--;
                result.add(copy);
            }
            ...
            for (SearchState ss : result) {
                ...
                List<StateItem> states2 = new LinkedList<>();
                nullableClosure(
                        next.item.the_production(), next.item.dot_pos(), next, states2, derivs2);
                finalizedResult.add(ss);
                for (int i = 1, size2 = derivs2.size(); i <= size2; i++) {
                    List<Derivation> subderivs2 = new ArrayList<>(derivs2.subList(0, i));
                    List<StateItem> substates2 = new ArrayList<>(states2.subList(0, i));
                    SearchState copy = ss.copy();
                    copy.derivs2.addAll(subderivs2);
                    copy.states2.addAll(substates2);
                    finalizedResult.add(copy);
        ...
        public int compareTo(SearchState o) {
            ...
            return reductionStreak(states1)
                    + reductionStreak(states2)
    ...
    protected static boolean compatible(symbol sym1, symbol sym2) {
        if (sym1 instanceof terminal) {
            ...
            if (sym2 instanceof terminal) return nt1.first_set().contains((terminal) sym2);
            non_terminal nt2 = (non_terminal) sym2;
    ...
    protected static boolean hasCommonPrefix(lalr_item itm1, lalr_item itm2) {
        ...
        for (int i = 0; i < dotPos; i++) if (rhs(prod1, i) != rhs(prod2, i)) return false;
        return true;
    }
